...
const AdminDashboard = () => {
  ...
  const [salesPagination, setSalesPagination] = useState({ page: 1, pages: 1, total: 0, limit: 50 });
  const [clientData, setClientData] = useState([]);
  const [clientPagination, setClientPagination] = useState({ page: 1, pages: 1, total: 0, limit: 50 });
  ...
  const fetchSalesData = useCallback(async (page = 1) => {
    try {
      setLoading(true);
      const response = await axios.get('/api/data/salesdata/admin/all', {
        params: { page, limit: salesPagination.limit, query: searchQuery }
      });
  ...
  }, [salesPagination.limit, searchQuery]);

  const fetchClientData = useCallback(async (page = 1) => {
    try {
      setLoading(true);
      const response = await axios.get('/api/data/clientdata/public/all', {
        params: { page, limit: clientPagination.limit, query: searchQuery }
      });
  ...
  }, [clientPagination.limit, searchQuery]);
  ...
  useEffect(() => {
    if (activeTab === 'utility') {
      ...
      fetchCustomers();
    } else if (activeTab === 'salesdata') {
      fetchSalesData(1);
    } else if (activeTab === 'clientdata') {
      fetchClientData(1);
    }
  }, [activeTab, fetchSalesData, fetchClientData]);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <button
            onClick={() => setActiveTab('salesdata')}
            className={`py-4 px-1 border-b-2 font-medium text-sm ${
              activeTab === 'salesdata'
                ? 'border-primary-500 text-primary-600'
                : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                ...
                <div className="px-4 py-5 sm:p-6">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex space-x-2">
                      <input
                        type="text"
                        placeholder="Search by Customer ID, Name, Paypro..."
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        className="px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
                      />
                      ...
                      <button
                        onClick={() => { setSearchQuery(''); fetchSalesData(salesPagination.page); }}
                        disabled={loading}
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          ...
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Paypro ID</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Date</th>
                        </tr>
                      </thead>
                      <tbody className="bg-white divide-y divide-gray-200">
                        {salesData.map((item) => (
                          <tr key={item._id}>
                            <td className="px-6 py-4 whitespace-nowrap">
                              ...
                              <div className="text-sm text-gray-500">{item.nameAndAddress}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.billMonth}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatCurrency(item.totalCost || 0)}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatCurrency(item.fpa || 0)}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatCurrency(item.gst || 0)}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatCurrency(item.retailTax || 0)}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatCurrency(item.incomeTax || 0)}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.payproId}</td>
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    <div className="text-sm text-gray-600">
                      Page {salesPagination.page} of {salesPagination.pages} — {salesPagination.total} records
                    </div>
                    <div className="flex gap-2">
                      <button
                        onClick={() => fetchSalesData(Math.max(1, salesPagination.page - 1))}
                        disabled={loading || salesPagination.page <= 1}
                        className="px-3 py-1 border rounded disabled:opacity-50"
                      >
                        Prev
                      </button>
                      <button
                        onClick={() => fetchSalesData(Math.min(salesPagination.pages, salesPagination.page + 1))}
                        disabled={loading || salesPagination.page >= salesPagination.pages}
                        className="px-3 py-1 border rounded disabled:opacity-50"
                      >
                        Next
                      </button>
                ...
                <div className="px-4 py-5 sm:p-6">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex space-x-2">
                      ...
                      <button
                        onClick={() => { setSearchQuery(''); fetchClientData(clientPagination.page); }}
                        disabled={loading}
                        className="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-md text-sm font-medium disabled:opacity-50"
                      >
                        Refresh
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          ...
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Type</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Marlas</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Value</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Receivable</th>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {clientData.map((row) => (
                          <tr key={row._id}>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row['Sr No.']}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.Phase}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.Block}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.AutoUnitNo}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.SaleNo}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.SaleDateISO ? new Date(row.SaleDateISO).toLocaleDateString() : ''}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.CustName}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.Type}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.Marlas}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatCurrency(row.Value || 0)}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatCurrency(row.ReceiveAble || 0)}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatCurrency(row.Received || 0)}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatCurrency(row.Balance || 0)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    <div className="text-sm text-gray-600">
                      Page {clientPagination.page} of {clientPagination.pages} — {clientPagination.total} records
                    </div>
                    <div className="flex gap-2">
                      <button
                        onClick={() => fetchClientData(Math.max(1, clientPagination.page - 1))}
                        disabled={loading || clientPagination.page <= 1}
                        className="px-3 py-1 border rounded disabled:opacity-50"
                      >
                        Prev
                      </button>
                      <button
                        onClick={() => fetchClientData(Math.min(clientPagination.pages, clientPagination.page + 1))}
                        disabled={loading || clientPagination.page >= clientPagination.pages}
                        className="px-3 py-1 border rounded disabled:opacity-50"
                      >
                        Next
                      </button>
